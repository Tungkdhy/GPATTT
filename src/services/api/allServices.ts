...
const mockFirewallConfigs = [
  ...
  { id: 3, name: 'SSH Access', type: 'Inbound', protocol: 'TCP', ports: '22', action: 'allow', priority: 90, description: 'Cho phép SSH từ admin network' },
];
...
export interface CreateFirewallConfigDto {
  name: string;
  type: string;
  protocol: string;
  ports: string;
...
export interface UpdateFirewallConfigDto {
  name?: string;
  type?: string;
  protocol?: string;
  ports?: string;
...
class FirewallConfigsService {
  async getAll(): Promise<FirewallConfig[]> {
    return new Promise((resolve) => {
      setTimeout(() => resolve(mockFirewallConfigs), 500);
    });
  }

  async getById(id: number): Promise<FirewallConfig> {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        const config = mockFirewallConfigs.find(c => c.id === id);
        if (config) {
          resolve(config);
        } else {
          reject(new Error('Firewall config not found'));
        }
      }, 300);
    });
  }
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        const newConfig: FirewallConfig = {
          id: mockFirewallConfigs.length + 1,
          ...data
        };
        resolve(newConfig);
      }, 500);
  ...
  async update(id: number, data: UpdateFirewallConfigDto): Promise<FirewallConfig> {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        const config = mockFirewallConfigs.find(c => c.id === id);
        if (config) {
